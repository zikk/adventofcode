...
function isLevelSafeDeep(values: number[]) {
  if (isLevelSafe(values)) return true;

  for (let i = 0; i < values.length; i += 1) {
    const truncated = values.filter((_, idx) => idx !== i);
    if (isLevelSafe(truncated)) return true;
  }
}
...
for (let i = 0; i < levels.length - 1; i += 1) {
  ...
  if (isLevelSafeDeep(level)) result += 1;
}
